    ...
    <div class="card-body" style="overflow-x: auto;">
        <div class="table-responsive" style="max-height: 400px;">
            <table class="table table-striped" id="table1">
                ...
                <tbody>
                    @foreach ($dailyquiz as $leaderboard)
                    <tr>
                        <td>{{ $leaderboard->id }}</td>
                        <td>{{ $leaderboard->user->name }}</td>
                        <td>{{ $leaderboard->test->title }}</td>
                        <td>{{ $leaderboard->marks_obtained }}</td>
                        <td>{{ $leaderboard->test->total_marks }}</td>
                        <td>{{ $leaderboard->test->level }}</td>

                        <td>
                            <div style="display: flex; align-items: center;">
                            <a href="{{route('student-delete',['id'=>$leaderboard->id])}}" onclick="confirmation(event)" style="margin-right: 10px;"><i class="bi bi-trash" style="color: red;font-size:20px"></i></a>
                        </div>
                        </td>
                    </tr>
                    @endforeach
