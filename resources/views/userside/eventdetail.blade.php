...
<?php
    ...
    $pdes = strip_tags($fetch->date);
    ?>
    <title> {{$fetch->title}}</title>
<meta name="description" content="{{$fetch->date}}" />
<meta property="og:image" content="http://hariomsimran.com/eventimages/{{$fetch->image}}" />
<meta property="og:url" content="http://hariomsimran.com/eventdetail/{{$fetch->slug}}" />
        ...
        <div class="row">
          <div class="col-md-8">
            ...
            <p>{!! html_entity_decode($fetch->description) !!}</p>
              ...
              <li>
                ...
                <div class="sidebar">
                        <?php
                                $url = urlencode("http://hariomsimran.com/eventdetail/$ptitle/$fetch->id");
          ...
          <div class="col-md-4">
            <div class="">
              <div class="item">
                <br><br>
                <img src="{{ asset('/uploads/event/' . $fetch->image) }}" alt="{{ $fetch->title }}">
              </div>
              <div class="item">
