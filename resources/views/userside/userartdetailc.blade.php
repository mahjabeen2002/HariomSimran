...
<head>
<?php
    $ptitle = str_replace(' ', '%20', $fetchart->headline);
    $pdes = strip_tags($fetchart->long_des);
    ?>
    <title> {{$fetchart->headline}}</title>
<meta name="description" content="{{$pdes}}" />
<meta property="og:image" content="http://hariomsimran.com/Clientarticleimages/{{$fetchart->image_video}}" />
<meta property="og:url" content="http://hariomsimran.com/userarticledetailc/{{$ptitle}}/{{$fetchart->id}}" />
  ...
  <div class="container">
    <div class="section-content">
      <div class="row">
        <div class="col-md-4">
          <div class="thumb">
            <img src="/Clientarticleimages/{{$fetchart->image}}" alt="" width="300px" height="300px">
          </div>
          <ul>
            <h3>Share On:</h3>
          <?php
                                $url = urlencode("http://hariomsimran.com/userarticledetailc/$ptitle/$fetchart->id");
        ...
        <div class="col-md-8">
          <h3 class="" style="font-size: 30px">{{$fetchart->headline}}</h3>
          <h5>{{$fetchart->art_date}}</h5>
          <p>{!! html_entity_decode($fetchart->long_des) !!}.</p>
          <p></p>
          <iframe width="560" height="315" src="{{$fetchart->video_url}}" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowfullscreen></iframe>
