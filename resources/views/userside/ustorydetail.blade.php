...
<head>
<?php
    $ptitle = str_replace(' ', '%20', $fetchsty->Title);
    $pdes = strip_tags($fetchsty->long_Des);
    ?>
    <title> {{$fetchsty->Title}}</title>
<meta name="description" content="{{$pdes}}" />
<meta property="og:image" content="http://hariomsimran.com/storyimages/{{$fetchsty->image_video}}" />
<meta property="og:url" content="http://hariomsimran.com/userstorydetail/{{$ptitle}}/{{$fetchsty->id}}" />
  ...
  <div class="container">
    <div class="section-content">
      <div class="row">
        <div class="col-md-4">
          <div class="thumb">
            <img src="/storyimages/{{$fetchsty->image}}" alt="" width="300px" height="300px">
          </div> <br>
          <h3>Share On:</h3>
          <?php
                                $url = urlencode("http://hariomsimran.com/userstorydetail/$ptitle/$fetchsty->id");
        ...
        <div class="col-md-8">
          <h3 class="name font-24 mt-0 mb-0" style="font-size: 40px">{{$fetchsty->Title}}</h3>
         <p>{!! html_entity_decode($fetchsty->long_Des) !!}.</p>

         <iframe width="560" height="315" src="{{$fetchsty->video_url}}" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowfullscreen></iframe>
