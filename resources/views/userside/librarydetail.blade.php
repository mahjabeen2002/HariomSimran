...
<head>
<?php
    $ptitle = str_replace(' ', '%20', $fetchmediade->title);
    $pdes = strip_tags($fetchmediade->long_des);
    ?>
    <title> {{$fetchmediade->title}}</title>
<meta name="description" content="{{$pdes}}" />
<meta property="og:image" content="http://hariomsimran.com/libraryimages/{{$fetchmediade->image}}" />
<meta property="og:url" content="http://hariomsimran.com/librarydetail/{{$ptitle}}/{{$fetchmediade->id}}" />
  ...
  <div class="container">
    <div class="section-content">
      <div class="row">
        <div class="col-md-4">
          <div class="thumb">
            <img src="/libraryimages/{{$fetchmediade->image}}" alt="" width="300px" height="300px">
          </div>
          <br>
          <ul>
            <h3>Share On:</h3>
          <?php
                                $url = urlencode("http://hariomsimran.com/librarydetail/$ptitle/$fetchmediade->id");
         ...
         <div class="sidebar">
          @if($fetchmediade->pdffile)
          <!-- Search Widget Start -->
          <div class="sidebar-widget widget-search">
            <h5 class="widget-title">Download Pdf</h5>
            <?php
            $a = str_replace(array( '[', ']','"','"' ), '', $fetchmediade->pdffile);
            $x = explode (",", $a);
        ?>
        @foreach($x as $key => $val)
        <form action="/download/{{$val}}" method="get">
            @csrf
              <div class="sigma_search-adv-input">
                ...
                <span>{{$val}}   <input type="submit" class="btn btn-sm pull-right" value="Download"></span>
        ...
        <div class="col-md-8">
          <h3 class="name font-24 mt-0 mb-0" style="font-size: 40px">{{$fetchmediade->title}}</h3>
          <?php

          $pv = DB::table('libcategories')->where('id',$fetchmediade->type)->first();
          ?>
          <h5 class="mt-5">Category : {{$pv->category_name}}</h5> 
          <p>{!! html_entity_decode($fetchmediade->short_Des) !!}.</p>
