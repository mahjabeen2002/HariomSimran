...
<dl class="">
            <dt style="color:teal">
              <?php
              $course = DB::table('category1tbls')->where('id',$totalmcq->skillid)->first();
              ?>
           <h4 style="text-align:center;color:teal"> {{$course->category_name}}</h4>

           <?php
           $fetch = DB::table('tadmintesttbls')->where('skillid',$totalmcq->skillid)->first();
           $time = explode(':',$fetch->time_dur)
           ?>

<p class="time" style="text-align: right">{{$fetch->time_dur}}</p>
...
<br>
<?php
$option = DB::table('tanswertbls')->where('quesid',$q->id)->first();
?>
