        ...
        <div class="page-heading">
                    ...
                    <div class="col-12 col-md-6 order-md-2 order-first">
                        <nav aria-label="breadcrumb" class="breadcrumb-header float-start float-lg-end">
                            <ol class="breadcrumb">
                                ...
                                <li class="breadcrumb-item active" aria-current="page">Hinduism Table</li>
                            </ol>
                        </nav>
                    </div>
                </div>
            ...
            <section class="section">
                <div class="card">
                    ...
                    <div class="">
                        <a href="{{ route('hinduism-create') }}" class="btn btn-success"
                            style="float: right; margin-top: -51px; margin-right: 41px;">Add Hinduism</a>
                            ...
                            <table class="table table-striped" id="table1">
                                <thead>
                                    <tr>
                                        ...
                                        <th scope="col">Hinduism ID</th>
                                        <th scope="col">Hinduism Title</th>
                                ...
                                <tbody>
                                    @foreach ($hinduism as $hinduism)
                                        <tr>
                                            <td>{{ $hinduism->id }}</td>
                                            <td>{{ $hinduism->title }}</td>

                                            <td>{{ $hinduism->description }}</td>
                                            <td>{{ $hinduism->videourl }}</td>
                                            <td>{{ $hinduism->status == '1' ? 'Hidden' : 'Visible' }}</td>
                                            <td><img style="height:50px; width:50px;"
                                                    src="{{ asset('/uploads/hinduism/' . $hinduism->image) }}"
                                                    alt=""></td>
                                            <td>
                                                <div style="display: flex; align-items: center;">
                                                    <a href="{{ route('hinduism-delete', ['id' => $hinduism->id]) }}"
                                                        onclick="confirmation(event)" style="margin-right: 10px;"><i
                                                            class="bi bi-trash" style="color: red;font-size:20px"></i></a>
                                                    <a href="{{ route('hinduism-edit', ['id' => $hinduism->id]) }}"><i
